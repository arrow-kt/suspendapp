...
actual object Unsafe {
  ...
  actual fun onShutdown(block: suspend () -> Unit): () -> Unit {
    suspend fun run(code: Int): Result<Unit> = runCatching {
      block()
      exitProcess(code)
    }.onFailure {
      it.printStackTrace()
  ...
  actual fun runCoroutineScope(
    ...
    suspend {
      ...
      val keepAlive: Job = innerScope.launch {
        while (isActive) {
          // Schedule a `no-op tick` by returning to main every hour with no actual work but just looping.
          delay(1.hours)
        }
      }
      runCatching {
